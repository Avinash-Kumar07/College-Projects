...
from umucv.htrans import htrans,row,jr,jc,col,sepcam
...
class zoomWindow:
    def __init__(self, wname, W=800, H=600):
        self.S = 0
        self.pos = [(0,0)]
        self.W = W
        self.H = H
    ...
    def update(self, key, image=None):
        ...
        if key == 171: xc=(W2-X)/s; yc=(H2-Y)/s; self.S += 1; s=2**self.S; self.pos[0]=(W2-xc*s, H2-yc*s)
        if key == 173: xc=(W2-X)/s; yc=(H2-Y)/s; self.S -= 1; s=2**self.S; self.pos[0]=(W2-xc*s, H2-yc*s)
...
class Video:
    def __init__(self, ext='mp4', codec='X264', fps=None):
        self.ext = ext
        self.codec = codec
    ...
    def write(self, frame, key=0, keypressed=ord('v')):
        ...
        if self.ON:
            if not self.Open:
                ...
                self.out = cv.VideoWriter(fname, fourcc, fps, (w,h))
                self.Open = True
