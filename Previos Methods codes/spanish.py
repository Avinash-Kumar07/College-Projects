...
def rgbh(xs,mask):
	...
	def h(rgb):
		return cv.calcHist([rgb], [0, 1, 2], imCropMask, [256//REDU, 256//REDU, 256//REDU], [0, 256] + [0, 256] + [0, 256])
		return normhist(sum(map(h, xs)))

def smooth(s,x):
	return gaussian_filter(x,s,mode ="constant")

bgsub = cv.createBackgroundSubtractorMOG2(500, 60, True) #threshold value may vary (60)
cap = cv.VideoCapture("ball.mp4")
key = 0
...
while(True):
	...
	if(camshift):
		cv.putText(frame,"CenterroiBox",(0,10), font, 0.5,(0,255,0),2,cv.LINE_AA)
		...
		# We apply camshift and draw the points on the screen
